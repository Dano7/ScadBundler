...
- **Line wrapping (`--max-line-length`, [ADR 0003](docs/adr/0003-max-line-length-wrapping.md))**: the emitter hard-wraps lines longer than `EmitOptions.MaxLineLength` at safe token boundaries (newlines are inter-token whitespace to OpenSCAD — CSG unchanged). Default **256 under `--minify`/`--obfuscate`** (some platforms' line-buffered `.scad` parsers overflow on longer lines; ≈1% size cost), **0 (off) otherwise**; `--max-line-length 0` = maximal minification. Breaks never land inside strings, `include` paths, comments, or a Customizer-annotated parameter line (`CommentParser.cc` reads the annotation off the assignment's first line).
